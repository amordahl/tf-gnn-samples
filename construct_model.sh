...
find ./ast_json -type f -name "*.ast" | parallel --progress python ../sv-comp-ml/scripts/ast-dump.py {} {}.json
echo 'Json files created.\nMoving the json files to the new model directory.'
find ./ast_json/train -type f -name "*.json" | parallel mv -t ./"$MODEL_DIR"/graphs-train
find ./ast_json/valid -type f -name "*.json" | parallel mv -t ./"$MODEL_DIR"/graphs-valid
find ./ast_json/test -type f -name "*.json" | parallel mv -t ./"$MODEL_DIR"/graphs-test

echo 'Zipping up files.'
find "$MODEL_DIR" -type f -name "*.json" | parallel gzip
# 4. Train model
echo 'Now training.'
